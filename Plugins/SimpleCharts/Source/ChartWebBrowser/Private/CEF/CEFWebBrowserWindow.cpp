...
FCEFWebBrowserWindow::~FCEFWebBrowserWindow()
{
	...
	if (IsValid())
	{
		UE_LOG(LogChartWebBrowser, Log, TEXT("Closing browser during destruction, this may cause a later crash."), *CurrentUrl);
	...
	UE_LOG(LogChartWebBrowser, Log, TEXT("Deleting browser for Url=%s."), *CurrentUrl);
}
...
#define LETTER_KEY_MACRO(val, vkey) else if(Key == EKeys::val) \
	...
	//UE_LOG(LogChartWebBrowser, Log, TEXT("Modifiers: %i %i %i") , OutKeyEvent.unmodified_character, OutKeyEvent.windows_key_code, OutKeyEvent.modifiers);
	...
	if (IsValid())
	{
		...
		if (bBlockTillClosed)
		{
			...
			while (InternalCefBrowser != nullptr)
			{
				...
				{
					UE_LOG(LogChartWebBrowser, Error, TEXT("CloseBrowser - took more than %0.2f second to close. Abandoning wait..."), CloseWaitTimeout);
...
void FCEFWebBrowserWindow::OnPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const void* Buffer, int Width, int Height)
{
	...
	if (bUsingAcceleratedPaint)
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("Accelerated CEF rendering selected but OnPaint called. Disabling accelerated rendering for this browser window."));
...
void FCEFWebBrowserWindow::OnAcceleratedPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, void* SharedHandle)
{
	...
	if (!bUsingAcceleratedPaint)
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("Accelerated CEF rendering NOT selected but OnAcceleratedPaint called. Enabling accelerated rendering for this browser window."));
